...
class OptILMClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, effort):
        ...
        effort_encoded = self.effort_encoder(
            effort.unsqueeze(1)
        )  # Shape: (batch_size, 64)
...
def preprocess_input(tokenizer, system_prompt, initial_query):
    ...
    # Support both old (encode_plus) and new (direct call) tokenizer APIs
...
def run(system_prompt, initial_query, client, model, **kwargs):
    try:
        ...
        predicted_approach, _ = predict_approach(
            router_model, input_ids, attention_mask, device
        )
        ...
        if predicted_approach == "none":
            ...
            return advanced_self_consistency_approach(
